...
- **MP3解码**: JLayer 1.0.1 (用于微软TTS引擎和MiniMax引擎的流式MP3解码)
...
└── ui/                                   // 【表现层】Jetpack Compose UI
    ...
    │   ├── MainScreen.kt                  // 主界面（含默认引擎提示、关于应用提示横栏、Demo 试听、引擎配置）
...
- **`TtsEngineApi`**: 定义标准行为（合成、停止、释放、声音查询、配置标签等），配套 `SynthesisParams` 封装合成参数。
- **`TtsSynthesisListener`**: 合成结果回调接口，支持音频数据流式回传、错误处理和完成通知。
- **`AbstractTtsEngine`**: 提供通用实现（日志、释放状态检查、可朗读文本检测）。
- **`TtsEngineRegistry`**: 引擎注册中心，以 `EngineIds` 密封类为单一数据源，管理所有可用引擎元信息。
- **`TtsEngineFactory`**: 引擎实例工厂，基于 DCL（双重检查锁定）延迟初始化注册表，统一管理 Engine、ConfigRepo、VoiceRepo 三种组件的创建，是插件化架构的核心入口。

#### 3.2.3 应用初始化架构
- **`TalkifyApplication`**: 应用入口类，负责：
  - 初始化 `TalkifyAppHolder` 全局上下文（供引擎等组件获取 Context）
...
- **`TalkifyAppHolder`**: 全局 Context 持有者，避免引擎层对 Android 组件的直接依赖
...
4. 委托给 `TtsEngineFactory.createEngine()` 创建的当前引擎实例。
5. 引擎通过网络流式请求音频数据，通过 `TtsSynthesisListener` 回调将音频数据回传给服务。
...
- **提示展示**：若非默认引擎，在 `MainScreen` 顶部显示提示横栏
...
- **声音数据**：各引擎支持的音色列表定义在 `res/xml/xxx-voices.xml` 中，通过 `VoiceXmlParser` 统一解析，便于独立更新。
...
> - **MiniMax 语音合成（minimax-tts）**：WebSocket 流式合成引擎，配置类为 `MiniMaxTtsConfig`，引擎实现为 `MiniMaxTtsEngine`，音色定义在 `minimax-voices.xml`，音频格式为 MP3。
